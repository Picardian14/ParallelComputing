...
unsigned int* mergeMultiplePares(unsigned int* buffer2,Tlista* container,unsigned int nroListas){
  ...
  for(p=pares;p>1;p=p/2){
	...
	for(i=0;i<pares;i+=2){ //pares === nrolistas /4 
...
unsigned int min, max;
 ...
 double tiempo_mergeMultiple = dwalltime() - timetick;
 //printf("%f ", tiempo_mergeMultiple);
 #ifdef PRINT
 ...
 double merge_mergeIncremental = dwalltime() - timetick;
 //printf("%f ", merge_mergeIncremental);
 #ifdef PRINT
...
//Reinicializa los indices para iterar las listas y pone en cero el arreglo de la mezcla
 ...
 double mergePares = dwalltime() - timetick;
 printf("%f %f %f", mergePares, merge_mergeIncremental, tiempo_mergeMultiple);
 printf("\n");
 #ifdef PRINT
  imprimirVector(mezcla,N, "Pares: ");
 #endif
