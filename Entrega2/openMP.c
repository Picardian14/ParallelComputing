...
double max (double * matriz, int N ){
    double vmax=-1;
    int i, numThreads;
    numThreads = omp_get_num_threads()/2;
    #pragma omp parallel for shared(matriz,N) private(i) num_threads(numThreads) reduction(max:vmax)
    for (i=0;i<N*N;i++)
        if (matriz[i] < vmax)
            vmax=matriz[i];
...
double min (double * matriz, int N ){
    ...
    int i, numThreads;
    numThreads = omp_get_num_threads()/2;
    #pragma omp parallel for shared(matriz,N) private(i) num_threads(numThreads) reduction(min:vmin)
    for (i=0;i<N*N;i++)
        if (matriz[i] < vmin)
            vmin=matriz[i];
...
double avg (double * matriz, int N ){
    ...
    int i, numThreads;
    numThreads = omp_get_num_threads()/2;
    #pragma omp parallel for shared(matriz,N) private(i) num_threads(numThreads) reduction(+:avg)
    for (i=0;i<N*N;i++)
        avg += matriz[i];
...
int main(int argc,char*argv[]){
 ...
 //#pragma omp for
  ...
  for(i=0;i<N;i++){
   #pragma omp parallel for private(j) firstprivate(i) num_threads(2)
   for(j=0;j<N;j++){
    #pragma omp parallel for private(k) firstprivate(i,j) num_threads(2)
    for(k=0;k<N;k++){
